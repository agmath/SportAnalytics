...
## Chapter 8: Principal Components Analysis and Clustering

Principal Components can be used to combine signals from correlated features in order to encode the majority of the information contained across those features (without the need to select only one or deal with multicollinearity). In this chapter, we look at PCA and clustering to identify styles of play and position groupings within players participating at the NFL Combine.

We'll start by scraping the combine data from the web.
...
for(year_idx in 2000:2023){
  url <- paste0("https://www.pro-football-reference.com/draft/", year_idx, "-combine.htm")
  ...
  combine_df <- combine_df %>%
    bind_rows(web_data_clean)
}

write.csv(combine_df, "combine_00_23.csv", row.names = FALSE)
```

The code above is marked to not be evaluated when the notebook is rendered. This, as usual with scraping, is meant to prevent pinging sites often and becoming rate-limited or blocked. When the data is updated (perhaps after the yearly combine), then we can run the code again to update our data. Otherwise, the data won't change and we'll simply read in the scraped data below.

```{r}
combine_df <- read_csv("combine_00_23.csv")

combine_df <- combine_df %>%
  separate(Ht, into = c("ht_ft", "ht_in"), sep = "-") %>%
  mutate(
    ht_ft = as.numeric(ht_ft),
    ht_in = as.numeric(ht_in),
    ht = 12*ht_ft + ht_in
  ) %>%
  select(-ht_ft, -ht_in)

names(combine_df) <- janitor::make_clean_names(names(combine_df))

combine_df
```

Let's look at variables which are associated with one another.
...
p2 <- combine_df %>%
  ggplot(aes(x = wt, y = x40yd)) + 
  geom_point() +
  geom_smooth(method = "lm")

p3 <- combine_df %>%
  ggplot(aes(x = broad_jump, y = vertical)) + 
  geom_point() + 
  geom_smooth(method = "lm")

p4 <- combine_df %>%
  ggplot(aes(x = wt, y = vertical)) + 
  geom_point() + 
  geom_smooth(method = "lm")
...
There are several other highly correlated pairs as well, such as 40-yard dash time and 3-cone time. Let's run PCA and check on the results.

```{r}
combine_df <- combine_df %>%
  filter(!is.na(ht))

pca_rec <- recipe(1 ~ ., data = combine_df) %>%
  step_rm(season) %>%
...
Note that the first principal component alone encodes nearly all the variation across the numeric features. We can plot the first two principal components, as seen in the text.
...
Per usual, there are differences here. It could be due to my scaling

```{r}
pca_rec %>%
  prep() %>%
  bake(combine_df) %>%
  bind_cols(combine_df) %>%
  ggplot() + 
  geom_point(aes(x = PC1, y = PC2, color = pos),
             alpha = 0.5) + 
  facet_wrap(~pos) + 
  theme(legend.position = "none")
```

Let's now use clustering on our combine data.

```{r}
kmeans_spec <- k_means(num_clusters = 6)
...
We can see summary statistics by cluster and position, if we like.
...
1. What happens when you do PCA onthe original data, with all of the NAs included? How does this affect the future football analytics workflow?
2. Perform any k-means clustering with the first three principal components, instead of only the first two. Do you notice differences? What about with the first four?
3. Perform the k-means clustering with five and seven clusters. How does this change the results?
4. Where else could we apply clustering as a component of an approach to football analytics?
